...
-(void)setCacheChannel:(NSString*)channelString {
    objc_setAssociatedObject(self, @selector(cacheChannel), channelString, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}

-(NSString*)cacheChannel{
    return objc_getAssociatedObject(self, @selector(cacheChannel));
}
                    ...
                    ^(NSData *data, NSURLResponse *response, NSError *error) {
                        ...
                        else
                        {
                            ...
                            [mgr backgroundTellDatabaseNamed: name to: ^(CBLDatabase *bgdb)
                            {
                                ...
                                NSError * error;
                                if (![self.cacheChannel isEqualToString:@""]) {
                                    requestCache.channels = self.cacheChannel;
                                }
